...
struct PagerankProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
    ...
    /**
     * Shuffle the map for evenly distributing the seeds
     */
    //template <typename keyT> keyT myrandom(keyT ii) {return std::rand() % ii;}
    vid_t myrandom(vid_t ii) {return std::rand() % ii;}
    struct RNG{
	int operator() (int n){
    ...
    void random_shuffle_map(std::map<vid_t, float> & m){
	std::vector<vid_t> v;
	...
	std::vector<vid_t>::iterator it = v.begin();
	for (auto & ii : m){
	    float ts = ii.second;
	    ii.second = m[*it];
	    m[*it] = ts;
...
#ifdef SEEDS
	    // For the seeds
	    //std::cout << "#############Choose seeds: " << seedMap.size() << std::endl;
